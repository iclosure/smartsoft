...
IMPLEMENT_DYNAMIC(CBCGPCircularGaugeScale, CBCGPGaugeScaleObject)
IMPLEMENT_DYNAMIC(CBCGPCircularGaugePointer, CBCGPGaugeDataObject)
...
void CBCGPCircularGaugeImpl::OnDraw(CBCGPGraphicsManager* pGMSrc, const CBCGPRect& /*rectClip*/, DWORD dwFlags/* = BCGP_DRAW_STATIC | BCGP_DRAW_DYNAMIC*/)
{
	...
	if (dwFlags & BCGP_DRAW_STATIC)
	{
		...
		if (m_bDrawTicksOutsideFrame)
		{
			...
			rectFill.DeflateRect(m_ScaleSize + 10, m_ScaleSize + 10);
		}
		...
		if (m_bShapeByTicksArea)
		{
			...
			pGM->DrawGeometry(m_GeometryShape1, m_Colors.m_brFrameOutline);
		...
		else
		{
			...
			pGM->DrawEllipse(rectFill, m_Colors.m_brFrameOutline);
		...
		if (m_nFrameSize > 2)
		{
			...
			rectInternal.DeflateRect(m_nFrameSize, m_nFrameSize);
			...
			else
			{
				...
				pGM->DrawGeometry(m_GeometryShape2, m_Colors.m_brFrameOutline);
			}
		...
		for (i = 0; i < m_arRanges.GetSize(); i++)
		{
				...
				pRange->m_dblOffsetFromFrame, pRange->m_nScale))
			{
				...
				pGM->DrawGeometry(pRange->m_Shape, pRange->m_brOutline);
			}
		}
	...
	{
		radius -= (m_nFrameSize + GetScale(0)->m_dblMajorTickMarkSize + GetScale(0)->m_dblOffsetFromFrame);
	}
	...
	if (dwFlags & BCGP_DRAW_DYNAMIC)
	{
		...
		for (i = 0; i < m_arSubGauges.GetSize(); i++)
		{
			...
			if (pGauge->IsVisible() && !pGauge->GetRect().IsRectEmpty())
			{
				pGauge->OnDraw(pGM, pGauge->GetRect(), BCGP_DRAW_DYNAMIC);
			}
		}

		if (m_bDrawTicksOutsideFrame)
		{
			radius -= m_ScaleSize + m_nFrameSize - GetScale(0)->m_dblMajorTickMarkSize;
		}
		...
		if (m_CapSize > 2.)
		{
			...
			rectCap.left = rectCap.CenterPoint().x - m_CapSize;
			rectCap.right = rectCap.left + 2 * m_CapSize;

			rectCap.top = rectCap.CenterPoint().y - m_CapSize;
			rectCap.bottom = rectCap.top + 2 * m_CapSize;
...
void CBCGPCircularGaugeImpl::OnDrawScale(CBCGPGraphicsManager* pGM, int nScale, const CBCGPPoint& center, double radius)
{
	...
	if (!brFill.IsEmpty() || !brOutline.IsEmpty())
	{
			...
			pScale->m_dblOffsetFromFrame, nScale))
		{
			...
			{
				pGM->DrawGeometry(pScale->m_Shape, brOutline);
			}
		}
	}

	radius -= pScale->m_dblOffsetFromFrame + 1;
	...
	{
		radiusTicks -= GetMaxLabelWidth(pGM, nScale) + m_nFrameSize + 1;
	}
		...
		dblVal += dblStep, i++)
	{
		...
		double dblCurrTickSize = bIsMajorTick ? pScale->m_dblMajorTickMarkSize : pScale->m_dblMinorTickMarkSize;

		if (bRecalcPositions)
		{
			double r = radiusTicks;

			if (!bIsMajorTick && m_bDrawTextBeforeTicks)
			{
				r -= dblCurrTickSize;
			}
		...
		if (bIsMajorTick)
		{
			...
			if (!strLabel.IsEmpty())
			{
				...
				if (bRecalcPositions)
				{
					...
					double xLabel = ptText.x - angleCos * 5.0;
					double yLabel = ptText.y + angleSin * 10.0;
				...
				if (!bSkipTickMark)
				{
					m_textFormat.SetDrawingAngle(pScale->m_bRotateLabels ? bcg_rad2deg(dblAngle) - 90. : 0.);
											...
											const CBCGPBrush& brFillIn, const CBCGPBrush& brOutlineIn)
{
	...
	const double dblSize = pScale->m_dblMajorTickMarkSize;
	...
	case CBCGPCircularGaugeScale::BCGP_TICKMARK_LINE:
		pGM->DrawLine(ptFrom, ptTo, brOutline);
		break;
	...
	case CBCGPCircularGaugeScale::BCGP_TICKMARK_BOX:
		{
			...
			pGM->DrawGeometry(rgn, brOutline);
		}
		break;
	...
	case CBCGPCircularGaugeScale::BCGP_TICKMARK_CIRCLE:
		{
			...
			pGM->DrawEllipse(ellipse, brOutline);
	...
	int nIndex)
{
	...
	if (pData->IsCircleShape())
	{
		...
		pGM->FillEllipse(ellipse, brFill);
		pGM->DrawEllipse(ellipse, brFrame);
	...
	{
		pGM->DrawLine(arPoints[i], arPoints[i + 1], brFrame);
	}

	if (nCount > 2)
	{
		pGM->DrawLine(arPoints[nCount - 1], arPoints[0], brFrame);
	}
}
//*******************************************************************************
void CBCGPCircularGaugeImpl::OnDrawPointerCap(CBCGPGraphicsManager* pGM, const CBCGPRect& rectCap)
{
	...
	pGM->DrawEllipse(rectCap, m_Colors.m_brCapOutline);
	...
	int nPointerIndex, BOOL bShadow)
{
	...
	if (dblSize == 0.0)
	{
		dblSize = dblRadius - (pScale->m_dblMinorTickMarkSize + pScale->m_dblOffsetFromFrame);
	}
	else
	{
	...
	{
		dblExtended = bcg_clamp(m_CapSize > 0.0 ? m_CapSize * 1.5 : dblSize / 4.0, 0.0, dblSize / 2.0);
	}
	...
	double dblWidth = bcg_clamp(pData->m_dblWidth, 0.0, dblRadius / 10.0);
	const double dblPointerAngle = dblAngle - M_PI_2;
	...
	case CBCGPCircularGaugePointer::BCGP_GAUGE_NEEDLE_ARROW_BASE:
		{
			...
			{
				double dblArrowLen = max(2.0 * dblWidth, 10.0);
				dblSize -= dblArrowLen;
	...
	double dblOffsetFromFrame, int nScale)
{
	...
	double r1 = radius - dblOffsetFromFrame - 1.;
	double r2 = max(.5 + m_CapSize, r1 - dblStartWidth);
	double r3 = max(.5 + m_CapSize, r1 - dblFinishWidth);
	...
	CBCGPComplexGeometry& shape, BOOL bInternal)
{
	...
	const double nCapSize = m_CapSize + m_nFrameSize / 2.0;
	...
	if (!bInternal)
	{
		const double a2 = m_nFrameSize / radius;

		radius += m_nFrameSize;

		centerM.x -= angleMiddleCos * m_nFrameSize;
		centerM.y += angleMiddleSin * m_nFrameSize;
...
void CBCGPCircularGaugeImpl::ReposSubGauge(CBCGPGraphicsManager* pGM, CBCGPGaugeImpl* pGauge, const CBCGPSize& sizeIn)
{
	...
	else
	{
		...
		case BCGP_SUB_GAUGE_TOP:
			pt.y = rect.top + (pt.y - rect.top - m_CapSize / 2.) / 2.;
			break;

		case BCGP_SUB_GAUGE_BOTTOM:
			pt.y = rect.bottom - (rect.bottom - pt.y - m_CapSize / 2.) / 2.;
			break;

		case BCGP_SUB_GAUGE_LEFT:
			pt.x = rect.left + (pt.x - rect.left - m_CapSize / 2.) / 2.;
			break;

		case BCGP_SUB_GAUGE_RIGHT:
			pt.x = rect.right - (rect.right - pt.x - m_CapSize / 2.) / 2.;
			break;
		}
...
double CBCGPCircularGaugeImpl::GetMaxLabelWidth(CBCGPGraphicsManager* pGM, int nScale)
{
		...
		dblVal += dblStep, i++)
	{
		...
		if (bIsMajorTick)
		{
			...
			dblMaxTextLen = max(dblMaxTextLen, sizeLabel.cx);
			dblMaxTextLen = max(dblMaxTextLen, sizeLabel.cy);
		}
	}
