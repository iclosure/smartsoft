											...
											BCGPChartFormatSeries::ChartCurveType curveType)
{
	...
	if (pSeries->GetChartType() == BCGP_CT_RANGE)
	{
		...
		if (ptTopStart.x == 0 && ptTopEnd.y == 0)
		{
...
CBCGPSize CBCGPLineChartImpl::OnCalcLegendKeySize(CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	int nMarkerSize = pSeries->GetSeriesFormat().m_markerFormat.m_options.GetMarkerSize();
	nMarkerSize = bcg_clamp(nMarkerSize, -9, 9);

	return CBCGPSize(28 * m_pRelatedChart->GetScaleRatio().cx, nMarkerSize * m_pRelatedChart->GetScaleRatio().cy);
}
//****************************************************************************************
CBCGPSize CBCGPLineChartImpl::OnCalcLegendKeySizeEx(const BCGPChartCellParams& params, CBCGPChartSeries* /*pSeries*/)
{
	...
	dblMarkerSize = bcg_clamp(dblMarkerSize, -9., 9.);
	
	return CBCGPSize(28 * params.GetScaleRatio().cx, dblMarkerSize * params.GetScaleRatio().cy);
}
//****************************************************************************************
		...
		CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	double dblDepth = CalcDepth3D(dblXUnitSize);
...
CBCGPSize CBCGPAreaChartImpl::OnCalcLegendKeySize(CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	return CBCGPSize(28 * m_pRelatedChart->GetScaleRatio().cx, 9 * m_pRelatedChart->GetScaleRatio().cy);
}
//****************************************************************************************
CBCGPSize CBCGPAreaChartImpl::OnCalcLegendKeySizeEx(const BCGPChartCellParams& params, CBCGPChartSeries* /*pSeries*/)
{
	return CBCGPSize(28 * params.GetScaleRatio().cx, 9 * params.GetScaleRatio().cy);
}
//****************************************************************************************s
						...
						CBCGPChartSurfaceTriangleID& triangleID, BOOL bIsGDI, BOOL bFill)
{
	...
	for (i = 0; i < triangle.m_arLevels.GetSize(); i++)
	{
		...
		int nNumPoints = (int)level.m_arLevelPoints.GetSize();
		...
		{
			CBCGPPoint pt1 = level.m_arLevelPoints[j];
			CBCGPPoint pt2 = level.m_arLevelPoints[(j + 1) % nNumPoints];
			...
			if (frameStyle != CBCGPChartSurfaceSeries::FS_CONTOUR_MESH)
			{
				if (triangle.m_bLeft)
				{
					...
					bPtOnEdge = triangle.CheckPointsOnEdge(pt1, pt2, 1, dblLinePrecision) || triangle.CheckPointsOnEdge(pt1, pt2, 2, dblLinePrecision);
				}
				else
				{
					// "external" edges - all except 2
					bPtOnEdge = triangle.CheckPointsOnEdge(pt1, pt2, 0, dblLinePrecision) || triangle.CheckPointsOnEdge(pt1, pt2, 1, dblLinePrecision);
				}
			...
			switch (frameStyle)
			{
			case CBCGPChartSurfaceSeries::FS_CONTOUR:
				if (!bPtOnEdge)
				{
					pGM->DrawLine(pt1, pt2, *pBrLine, dblLineWidth);
				}
				break;

			case CBCGPChartSurfaceSeries::FS_MESH:
				if (bPtOnEdge)
				{
					pGM->DrawLine(pt1, pt2, *pBrLine, dblLineWidth);
				}
			...
			case CBCGPChartSurfaceSeries::FS_CONTOUR_MESH:
				pGM->DrawLine(pt1, pt2, *pBrLine, dblLineWidth);
				break;
			}
		}
...
//****************************************************************************************
int SortTriangles(const void* pT1, const void* pT2)
{
							  ...
							  CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	else
	{
		...
		double dblDistance = dblDistanceFromMarker + 
			pSeries->GetDataPointLabelDistance(nDataPointIndex) * dblDistanceFromMarker / 100.;
...
CBCGPSize CBCGPLongDataChartImpl::OnCalcLegendKeySize(CBCGPChartSeries* /*pSeries*/, int /*nDataPointIndex*/)
{
	if (m_pRelatedChart == NULL)
	{
		return CBCGPSize(28, 9);
	}

	CBCGPSize szScale = m_pRelatedChart->GetScaleRatio();
	return  CBCGPSize(szScale.cx * 28, szScale.cy * 9);
}
//****************************************************************************************
