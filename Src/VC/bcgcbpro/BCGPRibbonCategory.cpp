...
void CBCGPRibbonTab::OnDraw (CDC* pDC)
{
	...
	CString strName = m_pParent->GetDisplayName();
...
int CBCGPRibbonCategory::GetMaxHeight (CDC* pDC)
{
	...
	return nMaxHeight + pDC->GetTextExtent (GetDisplayName()).cy + nPanelMarginTop + nPanelMarginBottom;
}
//******************************************************************************************
...
void CBCGPRibbonCategory::SetActive (BOOL bIsActive)
{
	...
	if (!m_pParenrRibbonBar->m_bIsPrintPreview || !globalData.DwmIsCompositionEnabled ())
	{
		ShowElements ();
	}
...
//*************************************************************************************
CString CBCGPRibbonCategory::GetDisplayName () const
{
	CBCGPRibbonBar* pRibbonBar = GetParentRibbonBar();
	if (pRibbonBar == NULL)
	{
	...
	CString strName;
	if (!pRibbonBar->m_CustomizationData.GetTabName(this, strName))
	{
		return m_strName;
	}
