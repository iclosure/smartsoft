								...
								BOOL bNCArea)
{
	...
	{
		CBCGPDrawManager dm (*pDC);
		dm.FillGradient(rectClient, m_clrMenuLight, m_clrMenuGutter);
											...
											CRect rect, COLORREF& clrText)
{
	...
	CBCGPDrawManager dm (*pDC);
	dm.Fill4ColorsGradient(rect, m_clrMenuItemGradient1, m_clrMenuItemGradient2, m_clrHighlightGradientDark, m_clrHighlightGradientDark);
											  ...
											  CBCGPBaseTabWnd* pBaseTab)
{
	...
	if (globalData.m_nBitsPerPixel <= 8 || globalData.IsHighContastMode () || pBaseTab->IsDialogControl () || pBaseTab->IsFlatTab ())
	{
										 ...
										 CPen& penLine)
{
	...
	{
		clrActiveTab =  pTabWnd->IsMDIFocused() && pTabWnd->IsActiveInMDITabGroup() ? m_clrHighlightDn : m_clrMenuLight;
	}
						...
						int iTab, BOOL bIsActive, const CBCGPBaseTabWnd* pTabWnd)
{
	...
	if (bIsActive || bIsHighlight || !bDefaultTabColor)
	{
		...
		if (clrTab == (COLORREF)-1)
		{
			if (pTabWnd->IsMDITab())
			{
				clrTab =  pTabWnd->IsMDIFocused() && pTabWnd->IsActiveInMDITabGroup() ? m_clrHighlightDn : m_clrMenuLight;
			}
			else
			{
		...
		if (bIsActive)
		{
			...
			{
				rectFill.bottom++;
				dm.FillGradient (rectFill, clrTab, RGB(255, 255, 255));
			}
			else
			{
...
//**********************************************************************************
void CBCGPVisualManagerVS2010::OnDrawTabCloseButton (CDC* pDC, CRect rect, 
											   ...
											   BOOL bIsDisabled)
{
	if (globalData.m_nBitsPerPixel <= 8 || globalData.IsHighContastMode () ||
		...
		pTabWnd->IsFlatTab () || pTabWnd->IsOneNoteStyle () || pTabWnd->IsVS2005Style () || pTabWnd->IsLeftRightRounded())
	{
		CBCGPVisualManagerVS2008::OnDrawTabCloseButton (pDC, rect, pTabWnd, bIsHighlighted, bIsPressed, bIsDisabled);
		return;
	}
	...
	CBCGPMenuImages::Draw (pDC, CBCGPMenuImages::IdClose, rect, 
			...
			BOOL bActive, CRect rectCaption, CRect rectButtons)
{
	...
	if (bActive)
	{
		dm.Fill4ColorsGradient(rectCaption, m_clrMenuItemGradient1, m_clrMenuItemGradient2, m_clrHighlightDn, m_clrHighlightDn);
	}
	else
	{
...
void CBCGPVisualManagerVS2010::OnUpdateSystemColors ()
{
	...
	m_clrComboDisabled = RGB(223, 230, 247);
...
{
	if (globalData.m_nBitsPerPixel <= 8 || globalData.IsHighContastMode () || pButton->IsHighlighted () || pButton->IsPressed ())
	{
...
void CBCGPVisualManagerVS2010::OnFillGridSelectAllAreaBackground (CBCGPGridCtrl* pCtrl, CDC* pDC, CRect rect, BOOL bPressed)
{
...

	{
		...
		dm.FillGradient 
			(
			rect, 
			m_clrMenuBarGradient1, 
			m_clrMenuBarGradient2
			);
	}
					...
					COLORREF& clrFill)
{
	...
	clrLine = bIsDisabled ? globalData.clrBarShadow : 
		(bIsPressed || bIsHighlighted) ? m_clrMenuItemBorder : RGB(255, 255, 255);
