									 ...
									 CBCGPGridRow::ClickArea* pnArea)
{
	...
	if (m_Rect.PtInRect (point))
	{
		...
		if (HasValueField () && !bIsAutoGroup && m_pWndList->m_nHorzScrollOffset > 0 && m_pWndList->GetColumnsInfo ().GetFrozenColumnCount () > 0)
...
void CBCGPGridRow::OnDrawRowMarker (CDC* pDC, CRect rect)
{
	...
	CBCGPMenuImages::DrawByColor(pDC, CBCGPMenuImages::IdArowRightTab3d, rect.TopLeft (), clr);
#else
									   ...
									   BOOL bIsGroupBox)
{
		...
		m_pWndList->m_bDragFromChooser) ) ) 
	{
		...
		{			
			...
			{
				if (rect.Width () >= m_pWndList->GetRowHeight () + cx) //m_pWndList->GetHeaderLineHeight ()
				{
					nArrow = m_pWndList->GetRowHeight (); //m_pWndList->GetHeaderLineHeight ()
				}
			}
	...
	else if (nSortVal != 0 && !bNoSortArrow)
	{
		...
		// Draw sort arrow:
		//-----------------
		int nArrowHeight = m_pWndList->GetRowHeight () - 2 * nArrowMargin;
...
COLORREF CBCGPGridCtrl::OnFillSelItem (CDC* pDC, CRect rectFill, CBCGPGridItem* /*pItem*/)
{
	...
	if (!m_bFocused && m_ColorData.m_SelColorsInactive.Draw (pDC, rectFill))
	{
...
BOOL CBCGPGridCtrl::OnDrawItem (CDC* pDC, CBCGPGridRow* pItem)
{
	...
	if (!pItem->m_Rect.IsRectEmpty ())
	{
		...
		if (pItem->m_Rect.bottom >= m_rectClip.top)
		{
			...
			BOOL bNoScrollCol0 = pItem->HasValueField () && !bIsAutoGroup && m_nHorzScrollOffset > 0 && GetColumnsInfo ().GetFrozenColumnCount () > 0;
				...
				!pItem->m_lstSubItems.IsEmpty ())
			{
				...
				rectExpand.right = rectExpand.left + m_nRowHeight + dx;
				rectExpand.DeflateRect (dx, 0, 0, 0);
