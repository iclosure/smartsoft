...
// indexes in array of edges
static int nPrimaryYAxisEdgeIdx = 5;
static int nPrimaryXAxisEdgeIdx = 7;

void PrintMatrix(const CBCGPMatrix4x4& m)
{
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
...
CBCGPPoint CBCGPChartDiagram3D::GetAxisPoint(int nAxisID, AxisPointType apt) const
{
	...
	case CBCGPChartDiagram3D::APT_FLOOR:
		if (edge.m_nOppositeEdgeIndex != (DWORD)-1)
		{
	...
	case CBCGPChartDiagram3D::APT_WALL:
		if (edge.m_nDiagonalEdgeIndex != (DWORD)-1)
		{
...
BOOL CBCGPChartDiagram3D::GetNormalAxisCoordinates(int nAxisID, CBCGPPoint& ptStart, CBCGPPoint& ptEnd)
{
	...
	for (int i = 0; i < m_edges.GetSize(); i++)
	{
		...
		if (edge.m_dwData == (DWORD)nAxisID)
		{
...
void CBCGPChartDiagram3D::StripWallsAndAxes(BOOL bStrip, BOOL bRedraw)
{
	...
	for (int i = 0; i < m_edges.GetSize(); i++)
	{
		...
		CBCGPChartAxis* pAxis = m_pChart->GetChartAxis(edge.m_dwData);
	...
	CBCGPBaseVisualObject(pContainer), m_currentTheme(CBCGPChartTheme::CT_DEFAULT)
{
	...
	m_titleAreaFormat.m_textFormat.Create(_T("Calibri"), 18);
...
BOOL CBCGPChartVisualObject::OnGetToolTip(const CBCGPPoint& pt, CString& strToolTip, CString& strDescr)
{
	...
	BOOL bHitResult = HitTest(pt, &hi, (DWORD)(BCGPChartHitInfo::HIT_AXIS | BCGPChartHitInfo::HIT_DATA_POINT | 
												BCGPChartHitInfo::HIT_AXIS_SCALE_BREAK));
	...
	else if (hi.m_hitInfo == BCGPChartHitInfo::HIT_DATA_POINT)
	{
		...
		if (bResult && strToolTip.GetLength() == 0)
		{
			strToolTip = _T(" ");
		}
		...
		m_rectLegendArea.PtInRect(pt) && (m_hitInfoFlags & BCGPChartHitInfo::HIT_LEGEND) != 0)
	{
		if ((m_hitInfoFlags & BCGPChartHitInfo::HIT_LEGEND_ENTRY) != 0)
		{
			for (int i = 0; i < m_arData.GetSize(); i++)
			{
				...
				if (pSeries == NULL || !pSeries->m_bVisible || !pSeries->m_bIncludeSeriesToLegend)
				{
					continue;
				}
		 ...
		 (m_hitInfoFlags & BCGPChartHitInfo::HIT_DATA_LABEL) != 0))
	{
		...
		for (i = (int)m_arData.GetSize() - 1; i >= 0 ; i--)
		{
			...
			if ((m_hitInfoFlags & BCGPChartHitInfo::HIT_DATA_POINT) != 0)
			{
				if (!IsChart3D())
				{
					if (!pSeries->GetAxesBoundingRect().NormalizedRect().PtInRect(pt))
					{
						continue;
					}
					...
					{
						const CBCGPChartDataPoint* pDataPoint = pSeries->GetDataPointAt(j);
						...
						CBCGPRect rectBounds = pSeries->GetDataPointBoundingRect(j);

						if (pSeriesFormat->m_markerFormat.m_options.m_bShowMarker)
						{
							rectBounds.InflateRect(pSeriesFormat->m_markerFormat.GetMarkerSize());
						}
						else
						{
							rectBounds.InflateRect(GetHitTestDataPointPrecision(TRUE));
						}
			...
			{
				for (int j = pSeries->GetMaxDataPointIndex(); j >= pSeries->GetMaxDataPointIndex(); j--)
				{
					const BCGPChartFormatSeries* pSeriesFormat = pSeries->GetDataPointFormat(j, FALSE);
...
CBCGPSize CBCGPChartVisualObject::CalcLegendSize(CBCGPGraphicsManager* pGM)
{
	...
	if (m_chartLayout.m_legendPosition == BCGPChartLayout::LP_NONE || pGM == NULL)
	{
		return szLegend;
	}
	...
	if (IsShowSurfaceMapInLegend())
	{
		...
		for (i = 0; i < m_arData.GetSize(); i++)
		{
			...
			if (pSurfaceSeries == NULL || !pSurfaceSeries->m_bVisible || !pSurfaceSeries->m_bIncludeSeriesToLegend)
			{
				continue;
			}
										...
										CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	if (pFormatSeries->m_curveType != BCGPChartFormatSeries::CCT_NO_LINE)
	{
		...
		pGM->DrawLine(ptStart, ptEnd, brLine, dblLineWidth,
			&pFormatSeries->m_seriesElementFormat.m_outlineFormat.m_strokeStyle);
	}
}
//****************************************************************************************
	...
	else
	{
		for (int j = 0; j < 2; j++)
		{
			for (i = 0; i < m_arData.GetSize(); i++)
			{
				...
				if (pSeries->IsBackgroundOrder() && j == 1 || 
					!pSeries->IsBackgroundOrder() && j == 0)
				{
					continue;
				}
...
void CBCGPChartVisualObject::OnDrawDiagramMarkers(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagram)
{
	for (int i = 0; i < m_arData.GetSize(); i++)
	{
		...
		if (pChartImpl != NULL)
		{
			if (pSeries->IsShownOnCustomOrResizedAxis())
			{
...
//****************************************************************************************
int CBCGPChartVisualObject::AddChartDataYX (double dblY, double dblX, int nSeries, DWORD dwUserData, BCGPChartFormatSeries* pFormatDataPoint)
{
...
//****************************************************************************************
int	CBCGPChartVisualObject::AddChartDataYXZ (double dblY, double dblX, double dblZ, int nSeries, DWORD dwUserData, BCGPChartFormatSeries* pFormatDataPoint)
{
	CBCGPChartSeries* pSeries = GetSeries (nSeries, TRUE);
	if (pSeries == NULL)
	{
...
//****************************************************************************************
int CBCGPChartVisualObject::AddChartDataYXY1 (double dblY, double dblX, double dblY1, int nSeries, DWORD dwUserData, BCGPChartFormatSeries* pFormatDataPoint)
{
	CBCGPChartSeries* pSeries = GetSeries (nSeries, TRUE);
	if (pSeries == NULL)
	{
...
//****************************************************************************************
int CBCGPChartVisualObject::AddChartDataYXY1 (const CString& strCategoryName, double dblY, double dblX, double dblY1, int nSeries, DWORD dwUserData, BCGPChartFormatSeries* pFormatDataPoint)
{
	CBCGPChartSeries* pSeries = GetSeries (nSeries, TRUE);
	if (pSeries == NULL)
	{
...
int CBCGPChartVisualObject::AddChartDataYY1 (const CString& strCategoryName, double dblY, double dblY1, 
											int nSeries, BCGPChartFormatSeries* pFormatDataPoint, DWORD dwUserData)
{
	CBCGPChartSeries* pSeries = GetSeries (nSeries, TRUE);
	if (pSeries == NULL)
	{
...
//****************************************************************************************
int CBCGPChartVisualObject::AddChartDataYY1Y2(double dblY, double dblY1, double dblY2, int nSeries, BCGPChartFormatSeries* pFormatDataPoint, DWORD dwUserData)
{
	CBCGPChartSeries* pSeries = GetSeries (nSeries, TRUE);
	if (pSeries == NULL)
	{
...
//****************************************************************************************
int CBCGPChartVisualObject::AddChartData (CArray<double, double>& arData, int nSeries, DWORD dwUserData, BCGPChartFormatSeries* pFormatDataPoint)
{
	ASSERT_VALID (this);
...
//****************************************************************************************
int CBCGPChartVisualObject::AddChartData (double dblData, int nSeries, CBCGPChartData::ComponentIndex ci, DWORD dwUserData, BCGPChartFormatSeries* pFormatDataPoint, 
...
//****************************************************************************************
int CBCGPChartVisualObject::AddChartData (double dblY, BCGPChartFormatSeries* pFormatDataPoint, int nSeries, DWORD dwUserData)
{
	ASSERT_VALID (this);
...
//****************************************************************************************
int  CBCGPChartVisualObject::AddChartData (const CString& strCategoryName, double dblY, int nSeries, BCGPChartFormatSeries* pFormatDataPoint, DWORD dwUserData)
{
	ASSERT_VALID (this);
...
//****************************************************************************************
int CBCGPChartVisualObject::AddChartEmptyData(int nSeries, DWORD dwUserData, BCGPChartFormatSeries* pFormatDataPoint, 
									   BCGPChartFormatSeries* pFormatSeries)
{
	ASSERT_VALID (this);
...
//****************************************************************************************
int CBCGPChartVisualObject::AddChartEmptyData(double dblX, int nSeries, DWORD dwUserData, BCGPChartFormatSeries* pFormatDataPoint)
{
	ASSERT_VALID (this);
...
//****************************************************************************************
int CBCGPChartVisualObject::AddChartEmptyData(const CString& strCategoryName, int nSeries, DWORD dwUserData, BCGPChartFormatSeries* pFormatDataPoint)
{
	ASSERT_VALID (this);
...
//****************************************************************************************
DWORD CBCGPChartVisualObject::GetDataPointUserData (int nSeries, int nDataPointIndex)
{
	ASSERT_VALID (this);
...
void CBCGPChartVisualObject::AddRelatedLegend(CBCGPChartLegendVisualObject* pLegend, BOOL bRedraw)
{
	...
	if (m_lstRelatedLegends.Find(pLegend) == NULL)
	{
