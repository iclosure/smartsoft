...
void CBCGPMessageBox::PreInitDialog ()
{
	...
	BOOL bDrawButtonsBanner = m_Params.bDrawButtonsBanner && !m_Params.bUseNativeControls && !globalData.IsHighContastMode();
	...
	AdjustControls (&m_arrButtons[0], (UINT)m_arrButtons.GetSize (), rectButtons, 
		m_arrButtons.GetSize () == 1 ? TA_CENTER : TA_RIGHT);
