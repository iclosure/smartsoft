...
BCGPGLOBAL_DATA::BCGPGLOBAL_DATA()
{
	...
	if (m_hinstUXThemeDLL != NULL)
	{
		...
		m_pfSetWindowTheme = 
			(BCGP_SETWINDOWTHEME)::GetProcAddress (m_hinstUXThemeDLL, "SetWindowTheme");
	}
	else
	{
	...
	if (m_hinstDwmapiDLL != NULL)
	{
		...
		m_pfDwmSetIconicThumbnail = 
			(BCGP_DWMSETICONICTHUMBNAIL) ::GetProcAddress(m_hinstDwmapiDLL, "DwmSetIconicThumbnail");

		m_pfDwmSetWindowAttribute = 
			(BCGP_DWMSETWINDOWATTRIBUTE) ::GetProcAddress(m_hinstDwmapiDLL, "DwmSetWindowAttribute");

		m_pfDwmSetIconicLivePreviewBitmap = 
			(BCGP_DWMSETICONICLIVEPRBMP) ::GetProcAddress(m_hinstDwmapiDLL, "DwmSetIconicLivePreviewBitmap");

		m_pfDwmInvalidateIconicBitmaps = 
			(BCGP_DWMINVALIDATEICONICBITMAPS) ::GetProcAddress(m_hinstDwmapiDLL, "DwmInvalidateIconicBitmaps");
	}
	else
	{
...
void BCGPGLOBAL_DATA::UpdateFonts()
{
	...
	// Create caption font:
	//---------------------
	lf.lfHeight = (long) ((1. + abs (lf.lfHeight)) * 3 / 2);
	if (lfHeightSaved < 0)
	{
		lf.lfHeight = -lf.lfHeight;
	}
