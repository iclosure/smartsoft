...
BOOL CBCGPMDIFrameWnd::PreTranslateMessage(MSG* pMsg) 
{
	...
	case WM_KEYDOWN:
		...
		{
			CBCGPBaseTabWnd* pWndTab = DYNAMIC_DOWNCAST(CBCGPBaseTabWnd, GetCapture());
			if (pWndTab->GetSafeHwnd() != NULL)
			{
				pWndTab->SendMessage (WM_CANCELMODE);
			}
		}
...
int CBCGPMDIFrameWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_Impl.m_bHasBorder = (lpCreateStruct->style & WS_BORDER) != 0;
