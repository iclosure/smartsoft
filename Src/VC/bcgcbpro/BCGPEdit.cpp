...
void CBCGPEdit::OnDrawBrowseButton (CDC* pDC, CRect rect, BOOL bIsButtonPressed, BOOL bHighlight)
{
	...
	if (m_bSearchMode && m_ImageSearch.IsValid() && m_bTextIsEmpty)
	{
		...
		{
			pDC->FillRect(rect, IsWindowEnabled() ? &CBCGPVisualManager::GetInstance ()->GetEditCtrlBackgroundBrush(this) : &globalData.brBtnFace);
		}
		else
		{
...
{
	BOOL bDrawPrompt = m_bSearchMode && m_bTextIsEmpty && !m_strSearchPrompt.IsEmpty() && (GetFocus() != this);
	...
	{
#ifndef _BCGSUITE_
		pDC->SetTextColor (m_bVisualManagerStyle ? CBCGPVisualManager::GetInstance ()->GetToolbarDisabledTextColor() : globalData.clrGrayedText);
#else
		...
		pDC->DrawText(m_strSearchPrompt, rectText, DT_LEFT | DT_SINGLELINE | DT_VCENTER);
