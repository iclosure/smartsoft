...
//	CString strStyleDLLName = _T("BCGPStyle2007") + strTheme + strVer + _T(".dll");
	CString strStyleDLLName = _T("Style2007") + strTheme + _T(".dll");
...
BOOL CBCGPVisualManager2007::OnNcPaint (CWnd* pWnd, const CObList& lstSysButtons, CRect rectRedraw)
{
	...
	if (dc.GetSafeHdc () != NULL)
	{
		...
		if ((pWnd->GetStyle () & WS_THICKFRAME) == 0)
		{
...
void CBCGPVisualManager2007::OnDrawPin(CDC* pDC, const CRect& rect, BOOL bIsPinned, BOOL bIsDark, BOOL bIsHighlighted, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	m_MenuItemPin.DrawEx (pDC, rect, bIsPinned ? 1 : 0, CBCGPToolBarImages::ImageAlignHorzCenter, CBCGPToolBarImages::ImageAlignVertCenter);
}
//*****************************************************************************
...
//**********************************************************************************
void CBCGPVisualManager2007::OnDrawTabCloseButton (CDC* pDC, CRect rect, 
											   const CBCGPBaseTabWnd* pTabWnd,
											   BOOL bIsHighlighted,
											   BOOL bIsPressed,
											   BOOL bIsDisabled)
{
	if (!CanDrawImage ())
	{
		CBCGPVisualManager2003::OnDrawTabCloseButton (pDC, rect, pTabWnd, bIsHighlighted, bIsPressed, bIsDisabled);
		return;
	}
					...
					CBCGPRibbonButton* pButton)
{
	...
	if (!bDisabled && pButton->IsBackstageViewMode () && 
		pButton->GetBackstageAttachedView() != NULL && !pButton->GetBackstageAttachedView()->GetRect().IsRectEmpty())
	{
		bIsMenuMode  = FALSE;
		bChecked     = TRUE;
		bHighlighted = FALSE;
					...
					COLORREF& clrText, COLORREF& clrSelBackground, COLORREF& clrSelText)
{
	...
	COLORREF color = GetRibbonEditBackgroundColor (pEdit, bIsHighlighted, bIsPaneHighlighted, bIsDisabled);
												  ...
												  BOOL bInfiniteMode)
{
	...
	{
		int count = m_ctrlRibbonProgressInfinity.GetImageCount ();
		double range = pProgress->GetRangeMax () - pProgress->GetRangeMin () + 1;

		range = (double)(pProgress->GetPos () - pProgress->GetRangeMin ()) / range;

		m_ctrlRibbonProgressInfinity.Draw (pDC, rectInternal, bcg_clamp((int)(range * count), 0, count - 1));
	}
}
//*******************************************************************************
