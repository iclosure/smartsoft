...
double CBCGPChartAxis::GetLabelDistance() const
{
	...
	{
		return (m_dblLabelDistance + 6) * (IsVertical() ? szScaleRatio.cx : szScaleRatio.cy);
	}
...
int CBCGPChartAxis::GetRightOffsetAsNumberOfValues() const
{
	...
	int nPixelsPerValue = GetFixedIntervalWidth() / GetValuesPerInterval();
...
double CBCGPChartAxis::ValueFromPointInternal(const CBCGPPoint& pt, CBCGPChartAxis::RoundType roundType) const
{
	...
	if (IsFixedIntervalWidth() && !m_bStretchFixedIntervals)
	{
		...
		return GetMinDisplayedValue() + dblPointOffset / dblValueStep;
	}
	...
	{
		dblVal = GetMinDisplayedValue() + (int)((dblVal - GetMinDisplayedValue() + ( m_dblMinAllowedMajorUnit / 2 ) ) / m_dblMinAllowedMajorUnit ) * m_dblMinAllowedMajorUnit;
	}
	else
	{
...
double CBCGPChartAxis::ValueFromPointOnAxisWithBreak(const CBCGPPoint& pt, CBCGPChartAxis::RoundType roundType) const
{
	...
	double dblVal = part.GetMinimumDisplayed() + dblRange / dblAxisSize * dblPointOffset;
	...
	if (roundType == CBCGPChartAxis::RT_FLOOR)
	{
		dblVal = dblMinimumDisplayed + (int)((dblVal - dblMinimumDisplayed) / part.m_dblMajorUnit) * part.m_dblMajorUnit;
	}
	else
	{
		dblVal = dblMinimumDisplayed + ((int)((dblVal - dblMinimumDisplayed) / part.m_dblMajorUnit) + 1) * part.m_dblMajorUnit;
	}
...
void CBCGPChartAxis::CalcMaxLabelSizeForScaleParts(CBCGPGraphicsManager* pGM)
{
	...
	if (m_scaleParts.GetCount() == 0 || m_axisLabelType == CBCGPChartAxis::ALT_NO_LABELS)
	{
		return;
	}
...
{
	if (m_pChart == NULL || !IsDiagram3D() || !IsAxisVisible())
	{
		return;
	}
	...
	if (GetComponentIndex() == CBCGPChartData::CI_X && dblXRotation >= 90 && dblXRotation <= 270 ||
		GetComponentIndex() == CBCGPChartData::CI_Y && (dblXRotation < 90 || dblXRotation > 270) ||
		GetComponentIndex() == CBCGPChartData::CI_Z && dblXRotation > 180)
	{
	...
	while (dblCurrValue <= dblMaxValue)
	{
		...
		if (bDrawNextLabel && 
			(bIs3D || bInterlaceLabels && nCount % nStep == 0 || !bInterlaceLabels || 
			 m_nDisplayedLines <= 3 && !IsFixedMajorUnit()))
		{
...
void CBCGPChartAxisX::CalcMaxLabelSize(CBCGPGraphicsManager* pGM)
{
	...
	if (m_axisLabelType == CBCGPChartAxis::ALT_NO_LABELS)
	{
		return;
	}
