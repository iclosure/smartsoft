...
IMPLEMENT_DYNAMIC(CBCGPLinearGaugePointer, CBCGPGaugeDataObject)
...
void CBCGPLinearGaugeImpl::OnDraw(CBCGPGraphicsManager* pGMSrc, const CBCGPRect& /*rectClip*/, DWORD dwFlags/* = BCGP_DRAW_STATIC | BCGP_DRAW_DYNAMIC*/)
{
	...
	if (dwFlags & BCGP_DRAW_STATIC)
	{
		...
		pGM->DrawRectangle(rect, m_Colors.m_brFrameOutline);
		...
		if (m_nFrameSize > 2)
		{
			...
			pGM->DrawRectangle(rectInternal, m_Colors.m_brFrameOutline);
		}
		...
		for (i = 0; i < m_arRanges.GetSize(); i++)
		{
				...
				pRange->m_dblOffsetFromFrame, pRange->m_nScale))
			{
				if (!rectRange.IsRectEmpty())
				{
					...
					pGM->DrawRectangle(rectRange, pRange->m_brOutline);
				}
				else
				{
					pGM->FillGeometry(shapeRange, pRange->m_brFill);
					pGM->DrawGeometry(shapeRange, pRange->m_brOutline);
				}

				m_dblMaxRangeSize = max(m_dblMaxRangeSize, max(pRange->m_dblStartWidth, pRange->m_dblFinishWidth));
			}
		}
	...
	if (!brFill.IsEmpty() || !brOutline.IsEmpty())
	{
			...
			0, nScale))
		{
			if (!rectRange.IsRectEmpty())
			{
				...
				pGM->DrawRectangle(rectRange, brOutline);
			}
			else
			{
				pGM->FillGeometry(shapeRange, brFill);
				pGM->DrawGeometry(shapeRange, brOutline);
			}
		}
	}
											...
											const CBCGPBrush& brFillIn, const CBCGPBrush& brOutlineIn)
{
	...
	const double dblSize = bMajor ? pScale->m_dblMajorTickMarkSize : pScale->m_dblMinorTickMarkSize;
...
void CBCGPLinearGaugeImpl::CreatePointerPoints(CBCGPPointsArray& arPoints, int nPointerIndex, BOOL bShadow)
{
	...
	if (dblSize == 0.0)
	{
		dblSize = max(pScale->m_dblMajorTickMarkSize, 2. * pScale->m_dblMinorTickMarkSize);
	...
	double offset = max(pScale->m_dblMajorTickMarkSize, pScale->m_dblMinorTickMarkSize);
	...
	const double delta = pData->m_dblWidth == 0 ? dblSize / 2 : pData->m_dblWidth / 2;
...
CBCGPSize CBCGPLinearGaugeImpl::GetDefaultSize(CBCGPGraphicsManager* /*pGM*/, const CBCGPBaseVisualObject* /*pParentGauge*/)
{
	if (m_bIsVertical)
	{
		return CBCGPSize(20., 100.);
	}
	else
	{
		return CBCGPSize(100., 20.);
	}
}
//*******************************************************************************
BOOL CBCGPLinearGaugeImpl::ValueToPoint(double dblValue, CBCGPPoint& point, int nScale) const
{
	...
	if (m_bIsVertical)
	{
		rect.DeflateRect(pScale->m_dblOffsetFromFrame + m_nFrameSize, m_sizeMaxLabel.cy / 2 + m_nFrameSize + 1);
	}
	else
	{
		rect.DeflateRect(m_sizeMaxLabel.cx / 2 + m_nFrameSize + 1, pScale->m_dblOffsetFromFrame + m_nFrameSize);
	}
