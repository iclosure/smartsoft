...
IMPLEMENT_DYNAMIC(CBCGPDiagramShape, CBCGPDiagramVisualObject)
IMPLEMENT_DYNAMIC(CBCGPDiagramCustomShape, CBCGPDiagramShape)
IMPLEMENT_DYNAMIC(CBCGPDiagramImageObject, CBCGPDiagramVisualObject)
IMPLEMENT_DYNAMIC(CBCGPDiagramTableShape, CBCGPDiagramShape)
...
void CBCGPDiagramShape::OnDraw(CBCGPGraphicsManager* pGM, const CBCGPRect& rectClip, DWORD dwFlags)
{
	...
	rectShadow.OffsetRect(nShadowDepth, nShadowDepth);
	...
	if (m_pGeometry != NULL)
	{
		...
		pGM->DrawGeometry(*m_pGeometry, m_brOutline);
	}
	...
	if (m_pGeometry == NULL)
	{
		...
		case Box:
			{
				CBCGPRoundedRect rrShadow(rectShadow, 5 * m_sizeScaleRatio.cx, 5 * m_sizeScaleRatio.cy);
				CBCGPRoundedRect rr(m_rect, 5 * m_sizeScaleRatio.cx, 5 * m_sizeScaleRatio.cy);
				...
				pGM->DrawRoundedRectangle(rr, m_brOutline);
			}
			break;
		...
		case Ellipse:
			...
			pGM->DrawEllipse(m_rect, m_brOutline);
			...
			if (nShadowDepth == 0)
			{
				...
				pGM->DrawEllipse(r1, m_brOutline);
			}
		...
		case Rectangle:
			{
				...
				pGM->DrawRectangle(rr, m_brOutline);
			}
			break;
	...
	case Star:
		{
			...
			if (bShadow)
			{
				arPoints += CBCGPPoint(nShadowDepth, nShadowDepth);
			}
	...
	case Cloud:
		{
			...
			if (bShadow)
			{
				pointStart += CBCGPPoint(nShadowDepth, nShadowDepth);
			}
...
//*******************************************************************************
CBCGPBaseVisualObject* CBCGPDiagramShape::CreateCopy () const
{
	return new CBCGPDiagramShape (*this);
}
...
//*******************************************************************************
void CBCGPDiagramCustomShape::CopyFrom (const CBCGPDiagramCustomShape& src)
{
...
void CBCGPDiagramCustomShape::OnDraw(CBCGPGraphicsManager* pGM, const CBCGPRect& rectClip, DWORD dwFlags)
{
	...
	for (i = 0; i < (int) m_arrGeometries.GetSize (); i++)
	{
		...
		if (pGeometry != NULL)
		{
			...
			pGM->DrawGeometry(*pGeometry, m_brOutline);
		}
	}
...
CBCGPGeometry* CBCGPDiagramCustomShape::CreateGeometry (CBCGPGraphicsManager* pGM, BOOL bShadow)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		{
			ptOffset += pPart->m_ptShadowOffset;
		}
...
//*******************************************************************************
CBCGPBaseVisualObject* CBCGPDiagramCustomShape::CreateCopy () const
{
	return new CBCGPDiagramCustomShape (*this);
}
//*******************************************************************************
...
void CBCGPDiagramImageObject::OnDraw(CBCGPGraphicsManager* pGM, const CBCGPRect& /*rectClip*/, DWORD dwFlags)
{
	...
	pGM->DrawRectangle (m_rect, GetOutlineBrush ());
...
//*******************************************************************************
CBCGPBaseVisualObject* CBCGPDiagramImageObject::CreateCopy () const
{
	return new CBCGPDiagramImageObject (*this);
}
...
void CBCGPDiagramTableShape::OnDraw(CBCGPGraphicsManager* pGM, const CBCGPRect& rectClip, DWORD dwFlags)
{
	...
	case Box:
		{
			CBCGPRoundedRect rrShadow(rectShadow, 5, 5);
...
// 				}
				...
				rectItemClip.bottom = rectItemClip.top + cyCaption + 2 * BCGP_DIAGRAM_TEXT_PADDING;
				rectItemClip.InflateRect (1.0, 1.0);
				rectItemClip.IntersectRect (rectClip);
		
				CBCGPRoundedRect rrCaption(rectItemClip, 5, 5);
				CBCGPGeometry captionGeometry;
				rectItemClip.top += rrCaption.radiusY + 1;
...
void CBCGPDiagramTableShape::DrawTextData(CBCGPGraphicsManager* pGM, const CBCGPRect& rectClip)
{
	...
	if (m_bCaption)
	{
		...
		pGM->DrawLine (rect.left, dy, rect.right, dy, m_brOutline);
	}
...
//*******************************************************************************
CBCGPBaseVisualObject* CBCGPDiagramTableShape::CreateCopy () const
{
	return new CBCGPDiagramTableShape (*this);
}
