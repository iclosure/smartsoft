	...
	if (!bInfiniteMode)
	{
		if (!rectChunk.IsRectEmpty () || pProgress->GetPos () != pProgress->GetRangeMin ())
		{
			...
			if (globalData.bIsWindowsVista)
			{
				nPart = PP_FILL;
				nState = PBFS_NORMAL;
				rectChunk.InflateRect (1, 1);
			}

			(*m_pfDrawThemeBackground) (m_hThemeProgress, pDC->GetSafeHdc(), 
										nPart, nState, &rectChunk, 0);
		}
	}
	else if (pProgress->GetRangeMin () != pProgress->GetRangeMax ())
	{
		...
		rectChunk.left += pProgress->GetPos() * rectChunk.Width () / (pProgress->GetRangeMax () - pProgress->GetRangeMin ());
		rectChunk.right = rectChunk.left + nChunkWidth;
		...
		(*m_pfDrawThemeBackground) (m_hThemeProgress, pDC->GetSafeHdc(),
				nPart, nState, &rectChunk, &rectProgress);
	}
}
//********************************************************************************
...
COLORREF CBCGPVisualManager2003::OnFillPropertyListSelectedItem(CDC* pDC, CBCGPProp* pProp, CBCGPPropList* pWndList, const CRect& rectFill, BOOL bFocused)
{
	...
	if (!pWndList->DrawControlBarColors())
	{
