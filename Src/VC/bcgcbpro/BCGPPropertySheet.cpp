...
void CBCGPPropertySheet::SetLook (PropSheetLook look, int nNavBarWidth, BOOL bGlassEffect)
{
	...
	if (m_look == PropSheetLook_AeroWizard)
	{
		...
		m_NavImages.Load (bIsHighDPI ? IDB_BCGBARRES_NAV_BUTTONS_120 : IDB_BCGBARRES_NAV_BUTTONS);
...
BOOL CBCGPPropertySheet::OnInitDialog() 
{
	...
	if (m_nHeaderHeight == 0 && ((m_psh.dwFlags & PSH_WIZARD) != 0) && IsVisualManagerStyle ())
	{
		m_nHeaderHeight = 1;
	}
...
void CBCGPPropertySheet::OnActivatePage (CPropertyPage* pPage)
{
	...
	else if (bIsCtrl)
	{
		...
		if (nNavBarWidth > 0)
		{
			...
			CRect rectPropSheet;
			GetClientRect(rectPropSheet);

			rectPage.right = rectPropSheet.right;

			rectPage.OffsetRect(nNavBarWidth, 0);
			rectPage.top = 0;
			rectPage.bottom = rectPropSheet.bottom;

			m_bIsReady = TRUE;

			pPage->SetWindowPos (NULL, rectPage.left, rectPage.top, rectPage.Width (),
				rectPage.Height (),
				SWP_NOZORDER | SWP_NOACTIVATE);
		}
	}
...
void CBCGPPropertySheet::DrawWizardAero(CDC* pDC)
{
	...
	if (m_nHeaderHeight > 0 && m_bDrawHeaderOnAeroCaption)
	{
		CRect rectBtnBack;
