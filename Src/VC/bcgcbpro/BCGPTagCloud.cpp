...
void CBCGPTagCloudElement::DoDraw(CBCGPTagCloud* pTagCloud, CBCGPGraphicsManager* pGM, CBCGPTextFormat& tf, BOOL bIsPressed, BOOL bIsHighlighted)
{
	...
	if (!m_rect.IsRectEmpty() && !m_strLabel.IsEmpty())
	{
		...
		CBCGPColor clrText = (bIsPressed || bIsHighlighted) ? m_colorHighlightedText : m_colorText;

		if (clrText.IsNull())
		{
			clrText = (bIsPressed || bIsHighlighted) ? pTagCloud->GetHighlightedTextColor() : pTagCloud->GetTextColor();
		}
