...
//	return _T("BCGPStyleScenic") + strVer + _T(".dll");
	return CString(_T("StyleScenic"))+ _T(".dll");
}
//*****************************************************************************
	...
	else if (pBar->IsKindOf (RUNTIME_CLASS (CBCGPPopupMenuBar)))
	{
			...
			!pBar->IsKindOf (RUNTIME_CLASS (CBCGPCalendarBar)))
		{
			...
			if (!pMenuBar->m_bDisableSideBarInXPMode)
			{
				...
				CBCGPPenSelector pen(*pDC, &m_penSeparatorDark);
	...
	CRect rect, CBCGPToolbarButton* pButton, CBCGPVisualManager::BCGBUTTON_STATE state, int cmd, CBCGPCalculator* pCalculator)
{
	...
	CBCGPControlRenderer& renderer = (cmd >= CBCGPCalculator::idCommandUser) ?
		m_ctrlRibbonBtnGroup_S :
		(cmd != CBCGPCalculator::idCommandNone && cmd != CBCGPCalculator::idCommandDot) ?
		m_ctrlRibbonBtnGroup_S : m_ctrlRibbonBtnGroup_S;

	renderer.Draw (pDC, rect, nIndex);
	return TRUE;
}
//********************************************************************************
