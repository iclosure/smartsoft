...
BOOL CBCGPTagManager::ReadDouble (const CString& strValue, double& value)
{
	...
	if (ExcludeTag (strValue, strItem))
	{
		...
		if (!strItem.IsEmpty ())
		{
			...
			value = atof(strItem);
#else
			char* astring = new char[strItem.GetLength () + 1];
			WideCharToMultiByte (CP_ACP, 0, strItem, -1, astring, strItem.GetLength () + 1, NULL, NULL);
			value = atof(astring);
			delete [] astring;
#endif
		}
...
CString CBCGPTagManager::FormatFont (const LOGFONT& value)
{
	...
	WriteTag (strValue, WriteInt (s_CharSet, value.lfCharSet, 0));
	WriteTag (strValue, WriteInt (s_OutPrecision, value.lfOutPrecision, 0));
	WriteTag (strValue, WriteInt (s_ClipPrecision, value.lfClipPrecision, 0));
	...
	WriteTag (strValue, WriteInt (s_PitchAndFamily, value.lfPitchAndFamily, 0));
