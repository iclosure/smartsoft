...
void CBCGPSmartDockingMarker::Create (SDMarkerPlace nSideNo, CWnd* pwndOwner)
{
	...
	CBCGPSmartDockingParams& params = CBCGPDockManager::GetSmartDockingParams ();
	const BOOL bAlphaMarkers = params.m_bIsAlphaMarkers || GetVMTheme () == BCGP_SDT_VS2008 || GetVMTheme () == BCGP_SDT_VS2010;
...
void CBCGPSmartDockingMarker::InitImages (CBCGPSmartDockingParams& params)
{
	...
	m_Image.SetAlwaysLight (uiResHotID == 0 && GetVMTheme () != BCGP_SDT_VS2010);
	...
	if (hwndBmp != NULL)
	{
		...
		if (!params.m_bIsAlphaMarkers && GetVMTheme () != BCGP_SDT_VS2008 && GetVMTheme () != BCGP_SDT_VS2010)
		{
...
void CBCGPSDCentralGroupMarker::Draw (CDC& dc, BOOL bAlpha)
{
	...
	if (bAlpha)
	{
		image.DrawEx (&dc, CRect (CPoint (xImage, yImage), image.GetImageSize ()), 0, 
			CBCGPToolBarImages::ImageAlignHorzLeft,
			CBCGPToolBarImages::ImageAlignVertTop,
			CRect (0, 0, 0, 0), (BYTE)(m_bHiLited ? 255 : ALPHA_TRANSPARENT));
		return;
	}
	...
	if (GetVMTheme () == BCGP_SDT_VS2008 || GetVMTheme () == BCGP_SDT_VS2010)
	{
		return;
	}
...
void CBCGPMarkerBmpWnd::OnPaint () 
{
	...
	if (!m_bIsDefaultImage || GetVMTheme () == BCGP_SDT_VS2008 || GetVMTheme () == BCGP_SDT_VS2010)
	{
		return;
	}
