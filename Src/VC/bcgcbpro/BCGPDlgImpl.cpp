...
HBRUSH CBCGPDlgImpl::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	...
	if (nCtlColor == CTLCOLOR_STATIC)
	{
		...
		if (strClass == STATIC_CLASS)
		{
			...
			if (m_bIsWhiteBackground && !globalData.IsHighContastMode())
			{
		...
		if (strClass == BUTTON_CLASS)
		{
			...
			return (HBRUSH) ::GetStockObject (m_bIsWhiteBackground && !!globalData.IsHighContastMode() ? WHITE_BRUSH : HOLLOW_BRUSH);
		}
	}
...
#if _MSC_VER < 1700 || !defined(_BCGSUITE_)
			BOOL bIsCompositionEnabled = globalData.DwmIsCompositionEnabled ();
#else
