...
IMPLEMENT_DYNAMIC(CBCGPDiagramVisualObject, CBCGPBaseVisualObject)
IMPLEMENT_DYNAMIC(CBCGPDiagramAnchorPointObject, CBCGPDiagramVisualObject)
IMPLEMENT_DYNAMIC(CBCGPDiagramConnector, CBCGPDiagramVisualObject)
IMPLEMENT_DYNAMIC(CBCGPDiagramGroupObject, CBCGPDiagramVisualObject)
IMPLEMENT_DYNAMIC(CBCGPDiagramTextDataObject, CBCGPVisualDataObject)
...
{
	return new CBCGPDiagramVisualObject (*this);
}
...
//*******************************************************************************
CBCGPBaseVisualObject* CBCGPDiagramAnchorPointObject::CreateCopy () const
{
	return new CBCGPDiagramAnchorPointObject (*this);
}
...
void CBCGPDiagramConnector::DrawConnection(CBCGPGraphicsManager* pGM, const CBCGPBrush& brOutline)
{
	...
	const double dLineWidth = m_bIsSelected ? 3.0 : 1.0;

	if (arPoints.GetSize () <= 2 || m_curveType == BCGP_CURVE_TYPE_LINE)
	{
		pGM->DrawLines (arPoints, brOutline, dLineWidth);
	}
	else
	{
		CBCGPSplineGeometry geometry(arPoints, CBCGPSplineGeometry::BCGP_SPLINE_TYPE_HERMITE, FALSE);
		pGM->DrawGeometry (geometry, brOutline, dLineWidth);
	}
}
//*******************************************************************************
void CBCGPDiagramConnector::DrawBeginArrow(CBCGPGraphicsManager* pGM, const CBCGPBrush& brOutline, const CBCGPBrush& brFill)
{
	...
	if (GetArrowPoints (arPoints[0], arPoints[1], m_arrowBegin, arArrowPoints))
	{
		...
		if (pGeometry != NULL)
		{
			...
			pGM->DrawGeometry (*pGeometry, brOutline);
			delete pGeometry;
		}
	}
							  ...
							  const CBCGPBrush& brOutline, const CBCGPBrush& brFill)
{
	...
	for (POSITION pos = m_mapTrackRects.GetStartPosition (); pos != NULL;)
	{
		...
		case HTBOTTOMLEFT:
			...
			pGM->DrawEllipse(rect, bLockedAnchor ? pContainer->GetAnchorOutlineBrush () :	brOutline);
			break;
			
		default:
			pGM->FillRectangle(rect, bLockedAnchor ? pContainer->GetAnchorFillBrush () :	brFill);
			pGM->DrawRectangle(rect, bLockedAnchor ? pContainer->GetAnchorOutlineBrush () :	brOutline);
		}
	}
}
//*******************************************************************************
...
//*******************************************************************************
CBCGPBaseVisualObject* CBCGPDiagramConnector::CreateCopy () const
{
	return new CBCGPDiagramConnector (*this);
}
...
//*******************************************************************************
void CBCGPDiagramTextDataObject::CopyFrom (const CBCGPDiagramTextDataObject& src)
{
	CBCGPVisualDataObject::CopyFrom (src);
...
CBCGPSize CBCGPDiagramTextDataObject::GetDefaultSize(CBCGPGraphicsManager* pGM, const CBCGPDiagramVisualObject* pParentObject)
{
	...
	if (m_textFormat.IsWordWrap())
	{
		if (pParentObject != NULL)
		{
			...
			dblWidth = pParentObject->GetRect ().Width () - 2 * BCGP_DIAGRAM_TEXT_PADDING;
		}
