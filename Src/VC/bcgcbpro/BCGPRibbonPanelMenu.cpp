...
BOOL CBCGPRibbonPanelMenuBar::OnSetAccData (long lVal)
{
	...
	if (m_pPanel != NULL)
	{
		...
		if (m_pPanel->IsMenuMode ())
		{
			int nIndex = (int)lVal - 1;
			int nCount = (int)m_pPanel->m_arElements.GetSize ();
			if (nIndex < nCount && nIndex >= 0)
			{
				CBCGPBaseRibbonElement* pElem = m_pPanel->m_arElements [nIndex];
				ASSERT_VALID (pElem);
				BOOL retVal = pElem->SetACCData(this, m_AccData);
				return retVal;
			}
...
HRESULT CBCGPRibbonPanelMenuBar::get_accChildCount(long *pcountChildren)
{
	...
	if (m_pPanel != NULL)
	{
		if (m_pPanel->IsMenuMode ())
		{
			*pcountChildren = m_pPanel->GetCount ();
		}
		else
		{
			CArray<CBCGPBaseRibbonElement*, CBCGPBaseRibbonElement*> arButtons;
