...
CSize CBCGPToolTipCtrl::GetIconSize ()
{
	...
	if (m_pRibbonButton != NULL)
	{
		...
		if (m_pRibbonButton->GetIcon (FALSE) != NULL)
		{
		...
		if (m_pRibbonButton->IsCustom())
		{
			CBCGPRibbonBar* pRibbonBar = m_pRibbonButton->GetTopLevelRibbonBar ();
			if (pRibbonBar != NULL)
			{
				sizeSmall = pRibbonBar->GetCustomElementImageSize(m_pRibbonButton, CBCGPRibbonButton::RibbonImageSmall);
