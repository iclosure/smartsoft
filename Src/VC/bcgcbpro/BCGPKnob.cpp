...
IMPLEMENT_DYNAMIC(CBCGPKnobPointer, CBCGPGaugeDataObject)
	...
	case CBCGPKnobPointer::BCGP_KNOB_POINTER_CIRCLE:
		{
			...
			if (dblWidth == 0.0)
			{
				dblWidth = max(2., dblRadius / 8);
			}

			dblOffset = dblRadius - 6. - dblWidth;
...
CBCGPSize CBCGPKnob::GetTickMarkTextLabelSize(CBCGPGraphicsManager* pGM, const CString& strLabel, const CBCGPTextFormat& tf)
{
	...
	return m_sizeIcon + m_sizeIconMargin * 2.;
}
//*******************************************************************************
void CBCGPKnob::OnDrawTickMarkTextLabel(CBCGPGraphicsManager* pGM, const CBCGPTextFormat& tf, const CBCGPRect& rectText, const CString& strLabel, double dblVal, int nScale, const CBCGPBrush& br)
{
	...
	{
		cx = m_sizeIcon.cx * cos(bcg_deg2rad(angle)) / 2;
	}

	CBCGPPoint ptImage(
		rectText.left + max(0., .5 * (rectText.Width() - m_sizeIcon.cx)) + cx, 
		rectText.top + max(0., .5 * (rectText.Height() - m_sizeIcon.cy)) + cy);

	pGM->DrawImage(m_Icons, ptImage, m_sizeIcon, 1., CBCGPImage::BCGP_IMAGE_INTERPOLATION_MODE_LINEAR,
