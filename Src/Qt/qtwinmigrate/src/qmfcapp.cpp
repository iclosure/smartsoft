...
static QMFCNativeEventFilter qmfc_nativeEventFilter;
...
#else
    QAbstractEventDispatcher::instance()->installNativeEventFilter(&qmfc_nativeEventFilter);
#endif
...
bool QMfcApp::winEventFilter(MSG *msg, long *result)
{
    ...
    QWidget *widget = QWidget::find(WID_CAST(msg->hwnd));
    HWND toplevel = 0;
    if (widget) {
        HWND parent = HWND_CAST(widget->winId());
        while(parent) {
            toplevel = parent;
