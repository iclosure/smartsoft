...
void QWinWidget::init() 
{
	...
	QT_WA({
        SetWindowLong(HWND_CAST(winId()), GWL_STYLE, WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS);
	}, {
        SetWindowLongA(HWND_CAST(winId()), GWL_STYLE, WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS);
	})
    SetParent(HWND_CAST(winId()), hParent);
