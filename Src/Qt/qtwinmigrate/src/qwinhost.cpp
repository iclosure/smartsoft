...
void QWinHost::fixParent()
{
    ...
    if (::GetParent(hwnd) == HWND_CAST(winId()))
        return;
    long style = GetWindowLong(hwnd, GWL_STYLE);
    if (style & WS_OVERLAPPED)
        return;
    ::SetParent(hwnd, HWND_CAST(winId()));
}
...
{
    QWinHost *widget = qobject_cast<QWinHost*>(QWidget::find(WID_CAST(::GetParent(hwnd))));
	...
	case WM_SYSKEYUP:
	    QT_WA({
        SendMessage(HWND_CAST(widget->winId()), msg, wParam, lParam);
	    }, {
        SendMessageA(HWND_CAST(widget->winId()), msg, wParam, lParam);
	    })
	    break;
...
bool QWinHost::event(QEvent *e)
{
    ...
    case QEvent::Polish:
        if (!hwnd) {
            hwnd = createWindow(HWND_CAST(winId()), qWinAppInst());
            fixParent();
            own_hwnd = hwnd != 0;
        }
        if (hwnd && !wndproc && GetParent(hwnd) == HWND_CAST(winId())) {
#if defined(GWLP_WNDPROC)
            QT_WA({
